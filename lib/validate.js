...
module.exports = {
  validate() {
    const getHandlerFileAndFunctionName = functionDefinition => {
      ...
      if (!imageProp?.command || imageProp.command.length < 1) {
    ...
    const processConfig = _config => {
      ...
      if (this.skipCompile) {
        ...
        if (!fs.existsSync(this.webpackConfig.output.path)) {
          return Promise.reject(new this.serverless.classes.Error('No compiled output found'));
        }
      ...
      if (!this.keepOutputDirectory) {
        ...
        fs.rmSync(this.webpackConfig.output.path, { recursive: true, force: true });
      }
    ...
    if (this.webpackConfig && _.isFunction(this.webpackConfig.then)) {
      return Promise.resolve(this.webpackConfig).then(config => processConfig(config));
    }

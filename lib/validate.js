    ...
    const processConfig = _config => {
      ...
      if (_.has(this.serverless, 'service.package') && this.serverless.service.package.individually) {
        ...
        this.webpackConfig = _.map(this.entryFunctions, entryFunc => {
          // handle array
          const config =
            _.isArray(this.webpackConfig) && this.webpackConfig.length
              ? _.cloneDeep(_.find(this.webpackConfig, c => c.entry[entryFunc.entry.key] !== undefined))
              : _.cloneDeep(this.webpackConfig);

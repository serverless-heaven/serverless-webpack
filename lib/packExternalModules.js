 ...
 * @this - The active plugin instance
 */
function getProdModules(externalModules, packagePath, dependencyGraph, forceExcludes, modulesPath) {
    ...
    if (moduleVersion) {
      ...
      try {
        const modulePackagePath = path.join(
          process.cwd(),
          modulesPath || path.join(path.dirname(packagePath), 'node_modules'),
          module.external,
          'package.json'
        );
...
module.exports = {
  ...
  packExternalModules() {
    ...
    const modulesPath = _.get(includes, 'modulesPath');
      ...
      return packager.getProdDependencies(path.dirname(packageJsonPath), 1).then(dependencyGraph => {
        ...
        const compositeModules = _.uniq(
          _.flatMap(stats.stats, compileStats => {
            ...
            return getProdModules.call(this, externalModules, packagePath, dependencyGraph, packageForceExcludes, modulesPath);
          })
        );

...
function getProdModules(externalModules, packagePath, tmpPackagePath) {
  ...
  // get the relative path from tmpPackagePath to packagePath
  const relativePackagePath = path.relative(path.dirname(tmpPackagePath), path.dirname(packagePath))
  externalModules.forEach(module => {

    let moduleVersion = packageJson.dependencies[module];

    if (moduleVersion) {
      // if moduleVersion is a relative path, rewrite it to be relative to tmpPackagePath
      if (moduleVersion.startsWith('file:')) {
        const modulePath = moduleVersion.split('file:')[1]
        if (!path.isAbsolute(modulePath)) {
          const newModulePath = path.join(relativePackagePath, modulePath)
          moduleVersion = `file:${newModulePath}`
        }
      }
...
module.exports = {
  packExternalModules(stats) {
    ...
    return BbPromise.resolve().then(() => {
      ...
      const prodModules = getProdModules(externalModules, packagePath, tmpPackageJson);

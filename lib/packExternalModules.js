 ...
 * @this - The active plugin instance
 */
function getProdModules(externalModules, packagePath, dependencyGraph, forceExcludes, modulesRoot) {
    ...
    if (moduleVersion) {
      ...
      try {
        const modulePackagePath = path.join(
          process.cwd(),
          modulesRoot || path.join(path.dirname(packagePath), 'node_modules'),
          module.external,
          'package.json'
        );
...
module.exports = {
  ...
  packExternalModules() {
    ...
    const modulesRoot = _.get(includes, 'modulesRoot');

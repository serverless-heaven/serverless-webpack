...
module.exports = {
  ...
  watchRun() {
    ...
    const deployFunctionsToLocalEmulator = require(
      path.join(this.serverless.config.serverlessPath, 'plugins', 'run', 'utils', 'deployFunctionsToLocalEmulator')
    );
    const getLocalRootUrl = require(
      path.join(this.serverless.config.serverlessPath, 'plugins', 'run', 'utils', 'getLocalRootUrl')
    );

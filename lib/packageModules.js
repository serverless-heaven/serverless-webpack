  ...
  copyExistingArtifacts() {
    if (isIndividialPackaging.call(this)) {
      ...
      _.forEach(allFunctionNames, funcName => {
        ...
        const artifactDestination = path.join('.serverless', `${funcName}.zip`);
        fs.copyFileSync(`.webpack/${funcName}.zip`, artifactDestination);
      ...
      const source = path.join('.webpack', `${name}.zip`);
      const destination = path.join('.serverless', `${name}.zip`);

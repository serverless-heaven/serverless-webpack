...
module.exports = {
  ...
  copyExistingArtifacts() {
    ...
    if (isIndividialPackaging.call(this)) {
      const allFunctionNames = this.serverless.service.getAllFunctions();
      _.forEach(allFunctionNames, funcName => {
        // Copy the articact from .webpack to support --no-build option
        const archiveName = `${funcName}.zip`;
        const artifactSource = path.join('.webpack', archiveName);
        const artifactDestination = path.join('.serverless', archiveName);
        fs.copyFileSync(artifactSource, artifactDestination);

        // Inform Serverless of where we copied the artifact to so it doesn't invoke packaging
        const func = this.serverless.service.getFunction(funcName);
        setArtifactPath.call(this, funcName, func, artifactDestination);
      });
    } else {
      // For service packaging, we should only have a single artifact
      const name = this.serverless.service.getServiceObject().name;
      const archiveName = `${name}.zip`;
      const source = path.join('.webpack', archiveName);
      const destination = path.join('.serverless', archiveName);

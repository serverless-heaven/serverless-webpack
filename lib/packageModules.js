...
module.exports = {
  ...
  packageModules(stats) {
    return BbPromise.mapSeries(stats.stats, (compileStats, index) => {
      ...
      const filename = entryFunction.funcName || this.serverless.serviceObject.name;
      const modulePath = compileStats.compilation.compiler.outputPath;

      const startZip = _.now();
      return zip.call(this, modulePath, `${filename}.zip`)
      ...
      .then(artifactPath => {
        if (_.get(this.serverless, 'service.package.individually')) {
          setArtifactPath.call(this, entryFunction.func, artifactPath);
        }
        return artifactPath;
      });
    })
    .then(artifacts => {
      if (!_.get(this.serverless, 'service.package.individually')) {
        ...
        _.forEach(allFunctionNames, funcName => {
          const func = this.serverless.service.getFunctions(funcName);
          setArtifactPath.call(this, func, artifacts[0]);
        });
      }

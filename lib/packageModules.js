...
module.exports = {
  packageModules() {
    ...
    return BbPromise.mapSeries(stats.stats, (compileStats, index) => {
      ...
      return zip
        ...
        .then(artifactPath => {
          if (this.isIndividialPackaging()) {
            setFunctionArtifactPath.call(this, entryFunction, artifactPath);
          }
          return artifactPath;
        });
    }).then(artifacts => {
      if (!this.isIndividialPackaging() && !_.isEmpty(artifacts)) {
  ...
  // TODO: Extract these out of module exports
  isIndividialPackaging() {
    return _.get(this.serverless, 'service.package.individually');
  },
  setServiceArtifactPath(artifactPath) {
    _.set(this.serverless, 'service.package.artifact', path.relative(this.serverless.config.servicePath, artifactPath));
  },
  setFunctionArtifactPath(funcName, artifactPath) {
    const func = this.serverless.service.getFunction(funcName);
    setArtifactPath.call(this, funcName, func, path.relative(this.serverless.config.servicePath, artifactPath));
  },
  copyExistingArtifacts() {
    if (!this.isIndividialPackaging()) {
      fs.copyFileSync('.webpack/service.zip', '.serverless/service.zip');
    } else {
      const allFunctions = this.serverless.service.getAllFunctions();
      _.forEach(allFunctions, func => {
        fs.copyFileSync(`.webpack/${func}.zip`, `.serverless/${func}.zip`);
      });
    }

    // TODO: Set context to inform Serverless of artifact paths

    return BbPromise.resolve();
  }
};
function setFunctionArtifactPath(entryFunction, artifactPath) {

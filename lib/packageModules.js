...
function setArtifactPath(funcName, func, artifactPath) {
  ...
  if (semver.lt(version, '1.18.0')) {
    ...
    this.serverless.cli.log(
      `${funcName} is packaged by the webpack plugin. Ignore messages from SLS.`,
    );
  } else {
    func.package = {
...
module.exports = {
  packageModules() {
    ...
    return BbPromise.mapSeries(stats.stats, (compileStats, index) => {
      ...
      const filename = `${entryFunction.funcName ||
      ...
      if (
        !_.get(this.serverless, 'service.package.individually') &&
        !_.isEmpty(artifacts)
      ) {

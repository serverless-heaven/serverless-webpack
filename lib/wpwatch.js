...
module.exports = {
  wpwatch() {
    ...
    const beforeCompile = () =>
      new Promise(resolve => {
        Promise.resolve(currentCompileWatch)
    ...
    const hasHooks = compiler.hooks?.beforeCompile;
    ...
    if (hasHooks) {
      ...
      compiler.plugin('before-compile', (_compilationParams, callback) => {
    ...
    const startWatch = callback => {
      ...
      compiler.watch(watchOptions, (err, stats) => {
        ...
        if (this.log) {
          this.log.verbose(`Webpack watch invoke: HASH NEW=${stats?.hash} CUR=${lastHash}`);
        } else {
        ...
        if (firstRun) {
          ...
          currentCompileWatch = Promise.resolve(this.serverless.pluginManager.spawn('webpack:compile:watch')).then(
